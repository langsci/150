...
\begin{tikzpicture}[node distance=4.3cm,>=stealth',bend angle=45,auto,every text node part/.style={align=center}]
  ...
  \begin{scope}
    % First net
    \node [place] (e2)             {\type{progression}};

    \node [placeout] (e1) [left of=e2] {\type{bounded}\\ \type{event}}
      edge [post] node[swap] {\feat{part-of}}  (e2);
  
    \node [place] (e3) [right of=e2] {\type{bounded}\\ \type{event}}
      edge [pre, bend right, pos=0.3, thick, dotted] node[swap] {\type{segm-of}}  (e1)
      edge [pre,thick, dotted] node[swap] {\it{part-of}}                          (e2)
      edge [pre, bend right, pos=0.6]  node[swap] {\feat{m-dim}}         (e2);
     
     \node [place] (l2) [below of=e1] {\type{degree}}
  		edge[pre, thick] node[swap] {\feat{max}} (e1)
 		edge[pre, thick] node[swap] {\feat{max}} (e3);
    
    \node [place] (e1init) [left of=e1] {\type{stage}}
      edge [pre] node[swap] {\feat{init}}  (e1);
      
%     \node [place] (l1)  [below of=e1init]  {\type{scale}}
 %    edge [pre] node[swap] {\feat{init}} (e1);
     
    \node [place] (l3)  [below of=l2] {\type{stage}}
       edge[post] node[swap] {\feat{sc-deg}} (l2)
 		edge[pre] node[swap] {\feat{fin}} (e3)
 		edge[pre, bend left] node[swap] {\feat{fin}} (e1);
 		
%     \node [place] (degree) [below of=l1] {\type{degree}}
 %     edge [pre] node[swap] {\feat{sc-deg}}  (l1);
      
       \node [place] (min) [right of=e3] {\type{degree}}
       edge [pre, bend right] node[swap] {\feat{min}}  (e1)

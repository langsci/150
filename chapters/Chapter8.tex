% Chapter 8
\chapter{Implementation of the analysis using XMG\index{XMG}} % Write in your own chapter title
\label{Chapter8}
%\lhead{Chapter 7. \emph{Implementation of analysis using XMG\index{XMG}}} 
In Chapter~\ref{Chapter7} I have proposed a frame semantic analysis\index{semantic analysis} of various prefixes together with selected pieces of the syntax-semantics interface\index{syntax-semantics interface}. In this chapter I present the implementation of the proposal.

 In order to describe and provide a compact grammar description, one can use a metagrammar\index{metagrammar} compiler. A TAG metagrammar\index{metagrammar} is a reduced description that captures linguistic generalizations that appear in the trees that belong to the grammar \citep{Candito:99}. EXtensible MetaGrammar\footnote{\url{http://xmg.phil.hhu.de/}} \citep[XMG\index{XMG}][]{Crabbe:13} is a formalism that allows to describe linguistic information contained in the grammar and a tool to compute grammar rules and produce a redundant strongly lexicalised TAG. 
 
Among the properties of XMG\index{XMG} that distinguish it from other grammar engineering environments, two are of particular importance for the current work. First, XMG\index{XMG} is a declarative language, which means that it is based on constraints and not on procedures. This allows for an order-independent definition of grammaticality. Second, XMG\index{XMG}'s notation is highly expressive: in particular, various linguistic dimensions are treated in a modular war, and grammatical units can be disjoint, conjoint, and inherited.
 
 XMG\index{XMG} 2 \citep{Petitjean:16} is a tool that is used to create metagrammar\index{metagrammar} compilers, adapting them to specific needs. Whereas XMG\index{XMG} supports three independent levels of description: syntactic trees \index{syntactic structure \index{syntactic structure}} (syn), semantic predicate structures (sem), and dynamic interfaces between syn and sem (dyn), XMG\index{XMG} 2 allows to introduce additional dimensions. The compiler I am using for the current implementation is created using XMG\index{XMG} 2 and has a syntactic (syn) and a frame semantic (frame) dimension  \citep{Lichte:15}.
...
The code starts with the three unicity constraints shown on \figref{xmg:unicity} that prevent the appearance of some features more than once in the same elementary tree\index{elementary tree}. The first constraint is a standard one, as it ensures that each tree has one lexical anchor\index{lexical anchor}. The second constraint has to be introduced because I use XMG\index{XMG} not only for constructing the unanchored trees, but also for the insertion of the lexical anchors. This constraint allows to make sure that only one noun is inserted in the accusative noun slot.
...
\end{lstlisting}
\caption{XMG\index{XMG} code: unicity constraints \label{xmg:unicity}}
\end{figure}
...
\end{lstlisting}
\caption{XMG\index{XMG} code: constructor of the cardinality dimension \label{xmg:cardinality}}
\end{figure}

\begin{figure}
% \begin{minipage}{0.5\textwidth}
% % \includegraphics[scale\index{scale}=1]{NounCardTree.pdf}\\
...
\end{lstlisting}
\caption{XMG\index{XMG} code for representation of the verb \textit{pisat'} `to write' \label{xmg:pisat}}
\end{figure}

The second group of lexical items consists of two verbs: \textit{pisat'} `to write' and \textit{zapisat'} `to write down'. The second verb contains the prefix \textit{za-} \index{suffix!za-@\textsl{za-}} , but its semantic contribution is not transparent, so the whole verb must be stored in the dictionary. The class that represents the verb \textit{pisat'} `to write' has a simple syntactic structure \index{syntactic structure} of two nodes (see \figref{xmg:pisat}): the node of category V and the node that contains the verb itself, where the V node inherits all syntactic properties of the verb, except for the category. The \textit{aspect} feature, in contrast with the features \textit{limited} and \textit{bounded}, is a syntactic feature and carries information about the syntactic aspect of the verb represented by the respective node. For the frame semantic side, I use a simple representation that serves the purposes of the current analysis. I acknowledge that the fully elaborated representation may be more complex or just differ in details, but this should not influence the results of the current study.


The syntactic structure \index{syntactic structure} of the prefixed verb \textit{zapisat'} `to write down/record' is more complex: the highest node is of category VP and under it a prefix node and another VP node are located. The internal VP node (VPInt in the code) is needed to make the structure of the dictionary-stored prefixed verb similar to the structure of prefixed verbs assembled in the metagrammar\index{metagrammar}. On the semantic side this verb also differs from the verb \textit{pisat'} `to write' a lot: it includes information about the measure dimension\index{scale!measure dimension}\index{scale!dimension} as well as about the initial and final stages of the event. The XMG\index{XMG} code of the class that represents the verb \textit{zapisat'} `to write down/record' is shown on \figref{xmg:zapisat} and the result of the compilation of the class is provided on \figref{fig:zapisat}.
...
?VPInt ?VPBase
{
  ...
  ;
  <frame>{
    ?X0[bounded-event\index{bounded event} & process,
        actor:?Actor,
        theme:?Theme,
        manner:[record],
        verb-dim:?X0,
        noun-dim:[property-scale\index{scale},
          min: ?ScMin,
          max: ?ScMax
        ],
        m-dim:[property-scale\index{scale},
          min: ?ScMin,
          max: ?ScMax
        ],
        init: [stage, 
          scale\index{scale}-deg:?ScMin
        ],
        fin: [stage, 
          scale\index{scale}-deg:?ScMax
        ]
    ]
  }
}
...
\end{lstlisting}
\caption{XMG\index{XMG} code for representation of the verb \textit{zapisat'} `to write down/record' \label{xmg:zapisat}}
\end{figure}
...
\section{Prefixes}
As we have already discussed the frames for all individual prefix usages in the previous chapter, I will not go through the code for all of them (it can be found in Appendix~\ref{AppendixB}), but show how frames correspond to the XMG\index{XMG} descriptions and what happens on the syntactic side, taking one prefix as an example.

Figure \ref{code:po} shows the XMG\index{XMG} description of the class for the prefix \textit{po-} \index{suffix!po-@\textsl{po-}} . In this code, the syntactic part represents a VP that consists of a prefix head and another (internal) VP that carries information about the derivational base\index{derivational base}. The agreement information as well as the semantic frame are then passed to the higher VP node. This node is also characterized by having perfective aspect (one may not call this aspect and consider aspect appearing at a later stage, but then this feature stores the value that will appear as soon as the aspect feature is initialized) independently of the value of the aspect feature of the internal VP node. Following the definitions provided above, the feature \textit{limited} is assigned the value \textit{yes} because the semantic frame contains the attribute \FIN, but the feature \textit{bounded} is assigned the value \textit{no}, as the value of the attribute \FIN is a free variable.
...
declare ?VP ?VPInt ?Po ?PoLex ?AGR ?X0 ?Init ?Fin ?VDim
{
  ...
  <frame>{
    ?X0[bounded-event\index{bounded event},
       m-dim: ?VDim,
       verb-dim: ?VDim,
       init: [stage, 
          scale\index{scale}-deg:?Init],
       fin: [stage, 
          scale\index{scale}-deg:?Fin]    
    ]
  }
}
\end{lstlisting}
\caption{XMG\index{XMG} code for the class describing the `delimitative \index{prefix interpretation!delimitative}' usage of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}} \label{code:po}}
\end{figure}

As for the frame description part, it follows straightforwardly earlier proposed frame configuration. To illustrate this, let us compare the code with \figref{frame:po:delim:rep} that shows the frame that was proposed in Chapter~\ref{Chapter7} for the delimitative \index{prefix interpretation!delimitative} usage of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}} . If one has a look on those two pictures, it becomes obvious that they differ only with respect to the variable names. 
...
\caption{Semantic contribution of \textit{po-} \index{suffix!po-@\textsl{po-}} \label{frame:po:delim:rep}}
\end{figure}

To make sure that the code not only looks similar to the frame, but also produces the desired result, let me show \figref{fig:poverb} that contains the result of the compilation of the proposed metagrammar\index{metagrammar} class.
...
%When one contemplates the frame description of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  it becomes clear that this prefix rarely can be used in combination with other prefixes or before the attachment of the secondary imperfective \index{imperfective aspect!secondary imperfective} suffix. If immediately after the attachment of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  another prefix is attached, the first step can be just skipped, as it increases the morphological complexity\index{morphological complexity} and does not influence the resulting semantic representation of the verb. Similar considerations apply if the \textit{po-} \index{suffix!po-@\textsl{po-}} prefixed verb is to be suffixed with the imperfective suffix \index{suffix!imperfective suffix} : normally, the basic imperfective verbs \index{imperfective aspect!imperfective verb \index{imperfective aspect!imperfective verb}} would denote the same events as the prefixed and suffixed verb. So even for the presence of some semantic difference, the price of adding two morphemes is too high and such verbs (as \textit{popisyvat'} `to write occasionally, non-seriously') would be used in a restricted number of situations when their meaning is pragmatically enriched. 
%
%If one looks at the situations when the prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  is attached to an already prefixed verb, the picture is also similar: if the last derivation step was prefixation \index{morphological operation!prefixation}, in many cases it has brought at least the semantic information the prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  contains, so further prefixation \index{morphological operation!prefixation} with \textit{po-} \index{suffix!po-@\textsl{po-}}  would be an unnecessary complication. This does not hold for those prefixes that do not overtly add information: e.g., if the verb \textit{zapisat'} `to write down' is suffixed with the suffix \textit{-iva-} in its progressive usage, the resulting verb is defined in terms of being a part of another (bounded) event, but it does not have its own boundaries. The prefix \textit{pere-} \index{suffix!pere-@\textsl{pere-}}  cannot add such boundaries, as it just copies information, so the verb \textit{perezapisyvat'} `to be rewriting' is imperfective and lacks boundaries, so they will be added when the prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  is attached. Similarly can be explained why the verb \textit{popriotkryt'} `to open very slightly' exists while in other cases an attachment of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  to a perfective verb \index{perfective aspect!perfective verb} does not seem to be possible (compare with an non-existent verb \textit{*pootkryt'}). In total, the predictions of a semantic-pragmatic analysis in many cases coincide with a syntactic restriction\index{restrictions on prefixation} (for \citealt{Tatevosov:13} the prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  is one of the selectionally limited prefixes \index{superlexical prefix!selectionally limited prefix \index{superlexical prefix!selectionally limited prefix}}, so it can attach only to imperfective verbs \index{imperfective aspect!imperfective verb \index{imperfective aspect!imperfective verb}}) and for some fragments such syntactic restriction\index{restrictions on prefixation} may be a fast way of limiting the number of derivation. As we see, however, the analysis proposed here allows for more accurate predictions in complicated cases.
%
%The distributive \index{prefix interpretation!distributive} interpretation of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  is acquired when the measure dimension\index{scale!measure dimension}\index{scale!dimension} is the cardinality scale\index{scale!cardinality scale}\index{scale}. 

Encoding of other prefix usages proceeds in the similar way: the syntactic part does not vary much from prefix to prefix and semantic descriptions can be directly obtained from the frame descriptions I have proposed in Chapter~\ref{Chapter7}. However, there are a couple of difficulties I want to discuss. First let us consider the prefix \textit{pere-} \index{suffix!pere-@\textsl{pere-}}  in the repetitive \index{prefix interpretation!repetitive} usage. There are several things that are different compared to the case of the `delimitative \index{prefix interpretation!delimitative}' usage of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}} . First, the value of the features \textit{aspect} and \textit{bounded} is inherited from the  lower VP and the value of the \textit{limited} feature of the derivational base\index{derivational base} has to be \textit{yes}. Second, at the moment of prefix attachment the central node of the frame shifts: derived VP (node \texttt{?VP} on \figref{code:pere}) is related to the frame \texttt{?X1} whereas the semantics of the derivational base\index{derivational base} is represented by the frame \texttt{?X0} (subframe of \texttt{?X1} on \figref{code:pere}). This realizes the solution proposed in the previous chapter. 
...
?Aspect ?EventType ?Init ?Fin ?Asp
{
  ...
  <frame>{
    ?X1[?EventType,
       m-dim:?Scale[property-scale\index{scale}],
       noun-dim:?NounDim,
       init: ?Init,
       fin: ?Fin,
       prep:?X0[?EventType,
...
\end{lstlisting}
\caption{XMG\index{XMG} code for the class that describes the repetitive \index{prefix interpretation!repetitive} usage of the prefix \textit{pere-} \index{suffix!pere-@\textsl{pere-}} \label{code:pere}}
\end{figure}

In order to perform the coercion\index{coercion} that is needed when the prefix \textit{pere-} \index{suffix!pere-@\textsl{pere-}}  is attached to a simplex imperfective verb \index{imperfective aspect!imperfective verb}, a separate step is required. It is realised by the class \texttt{NDimCoercedVerb} (see \figref{xmg:coersion}) that transforms a non-bounded event\index{bounded event} into a bounded event\index{bounded event} using the nominal scale\index{scale}.
...
declare ?VP ?AGR ?X0 ?ScMin ?ScMax ?NounDim ?VPInt
{
  ...
  <frame>{
    ?X0[bounded-event\index{bounded event},
       m-dim: ?NounDim[property-scale\index{scale} & closed-scale\index{scale!totally closed}\index{scale},
          min: ?ScMin,
          max: ?ScMax],
       noun-dim:?NounDim,
       init:[stage, 
...
\end{lstlisting}
\caption{XMG\index{XMG} code for the class that implements coersion of an unbounded event into a bounded event\index{bounded event}\label{xmg:coersion}}
\end{figure}

\section{imperfective suffix \index{suffix!imperfective suffix} }
I use two separate classes to produce two interpretations of secondary imperfective \index{imperfective aspect!secondary imperfective} verbs \index{imperfective aspect!imperfective verb \index{imperfective aspect!imperfective verb}}: progressive and habitual. For the analysis I propose it is important to distinguish between them when another prefix is attached after the suffixation \index{morphological operation!suffixation} , as these two interpretations have different semantic properties.

The habitual interpretation of the imperfective suffix \index{suffix!imperfective suffix} , realised by the code shown on \figref{xmg:habitual}, produces an unlimited event that is a series of \index{imperfective aspect!habitual intepretation} limited events. The \NOUNDIM of the new event necessarily is of type \textit{cardinality} and does not need to correspond to the respective attribute of the derivational base\index{derivational base}. The verbal dimension\index{verbal dimension} is copied from the individual event level to the series level. This interpretation of the imperfective suffix \index{suffix!imperfective suffix}  is also associated with the introduction of the \textit{iteration} type of the event and the respective syntactic feature. The result of the compilation of this class is shown on \figref{fig:iter}.
...
declare ?VP ?VPInt ?Suf ?Iva ?AGR ?X0 ?X1 ?VDim
{
  ...
  <frame>{
    ?X1[event & iteration,
       segment:?X0[bounded-event\index{bounded event},
         noun-dim:[property-scale\index{scale}],
         verb-dim: ?VDim],
       verb-dim: ?VDim,
...
\end{lstlisting}
\caption{XMG\index{XMG} code for the habitual interpretation of the imperfective suffix \index{suffix!imperfective suffix}  \label{xmg:habitual}}
\end{figure}
...
The second interpretation of the imperfective suffix \index{suffix!imperfective suffix}  is progressive: on the semantic side I represent it as a creation of a new event that is a \PARTOF the event denoted by the derivational base\index{derivational base}. Due to the \PARTOF relation the new event remains limited. On Figures~\ref{graph:pf} and~\ref{graph:ipf} in Chapter~\ref{Chapter1} I have realised \textit{part of} as a relation, as in this case (in contrast to the prefix \textit{do-} \index{suffix!do-@\textsl{do-}} ) it is not functional. As relations are currently not implemented in XMG\index{XMG}, for the sake of the implementation I use \PARTOF as an attribute when representing the progressive interpretation \index{imperfective aspect!progressive interpretation} of the imperfective suffix \index{suffix!imperfective suffix} .

\section{Assembling the parts}
The last part of the code assembles the verbal phrases from the components described above. As the resource has to be finite, recursion is not allowed in the XMG\index{XMG} class descriptions.  Due to this restriction, it is not possible to define a single class that would allow an arbitrary number of prefixes to be stacked (by the possibility of attaching a new prefix to the output of the same class). This means that each prefixation \index{morphological operation!prefixation} level has to be described separately. First three classes do the job of assembling verbs with one prefix: the first class (\texttt{OneBasePrefixedVerb}) combines a simplex verb and one of the prefixes; the second class (\texttt{OneCoercedPrefix\-edVerb}) combines a coerced verb with one of the prefixes \textit{pere-} \index{suffix!pere-@\textsl{pere-}}  (repetitive \index{prefix interpretation!repetitive} interpretation) and \textit{do-} \index{suffix!do-@\textsl{do-}} ; the last class (\texttt{VerbWithOnePrefix}) assembles under one name the results of the first two classes and all prefixed verbs that are stored in the dictionary. 
...
\end{lstlisting}
\caption{XMG\index{XMG} code for the verbs with two prefixes \label{xmg:2pref}}
\end{figure}
...
\end{lstlisting}
\caption{XMG\index{XMG} code for the operation of type matching \label{xmg:typemather}}
\end{figure}

I allow for one more derivational step in the described fragment: attachment of a prefix after suffixation \index{morphological operation!suffixation} . This is performed by the class \texttt{TwoPrefixedSuffixedVerb} that uses the result of the compilation of the \texttt{SuffTyped} class and all available prefix classes. At this moment all possible verbal models are created. Then the next step of combining those models with various interpretations of the direct object is performed.

This step is done by two classes: \texttt{PrefixedVerbDirObj} and \texttt{PrefixedSuffixedVerbDirObj} that take, respectively, prefixed and prefixed-suffixed verbs, and all available dimension constructors\index{dimension constructor\index{dimension constructor}}. An output of those classes are models of all possible VPs that use all the available scalar interpretations of the direct objects. This output is again combine with the \texttt{TypeMatcher} class to ensure proper type inheritance.
...
%For the piece I present I need two dimension constructors\index{dimension constructor\index{dimension constructor}}: one referring to the length of the story and another to the cardinality of the set of stories. Both constructors are based on the attributes that are present in the object description: e.g., if there is no \textit{length} attribute in the object frame, the maximum value\index{scale!maximum value}\index{scale!maximum value\index{scale!maximum value}} of the measure dimension\index{scale!measure dimension}\index{scale!dimension} will be undefined. This is controlled by selecting only appropriate nouns (with a relevant attribute) as those that can undergo the enrichment procedure. Again, as I have to delegate all work to the metagrammar\index{metagrammar} compiler, I have to describe such restrictions in the metagrammar\index{metagrammar} (see the first line in the class specification), but normally they would be realised as separate metagrammar\index{metagrammar} classes and elementary trees for nouns would be accompanied with the information about the possibility of being inserted as lexical anchors in such classes.
%
%\begin{figure}
%\begin{lstlisting}[style=xmg]
...
%}
%
%%% Plural nouns can be interpreted as introducing a cardinality scale\index{scale!cardinality scale}\index{scale}
%class NounCardinal
%export ?N
...
%}
%\end{lstlisting}
%\caption{Nominal dimension\index{nominal dimension} constructors\index{dimension constructor\index{dimension constructor}}: length and cardinality}
%\end{figure}
%
...
\begin{table}
\caption{Output of the XMG\index{XMG} processing for the class of one- or two-prefixed verbs\label{table:prefverbs}}
\resizebox{\textwidth}{!}{\begin{tabular}{lllll}
\lsptoprule
verb  & semantics & noun  & exists  & blocked by \\
      &           & interpretation & &    pragmatics\index{pragmatics} \\     \midrule
...
Note that already at this stage XMG\index{XMG} reduces 40 possible models (five variants of the first prefix, four variants of the second prefix, and two interpretations of the noun) to only 10 (seven correct and three non-existent) by an appropriate combination of constraints. 

Now let us have a look at the next step: when the verbs from the list above get suffixed with the imperfective suffix \index{suffix!imperfective suffix}  (in one of two interpretations). The output of this part consists of 23 verbs out of which only two must be filtered out as they are produced on the basis of the verbs that, as we have discussed above, do not exist: \textit{perepopisat'} `to write all of' and \textit{popopisat'} `to write for some time'. It is interesting to note that the second interpretation of the last verb does not get suffixed, so the number of wrong models on the new level does not get multiplied (by the two possible interpretations of the suffix). Instead of the six potential incorrect models just on the basis of the wrong predictions of the previous level we obtain only two. All the verbs produced by this part of the implementation are shown in Table~\ref{table:suffverbs} together with their English translations, interpretation of the imperfective suffix \index{suffix!imperfective suffix} , and information about existence.\largerpage

\begin{table}
 \caption{Output of the XMG\index{XMG} processing for the class of prefixed and then suffixed verbs\label{table:suffverbs}}
...
The last group of verbs consists of 49 models that contain at least one prefix attached before the imperfective suffix \index{suffix!imperfective suffix}  and at least one prefix attached after it. They are shown in Table~\ref{table:prefsuffverbs} together with English translations (not always exact) and the aspect (as here some of the verbs, despite being prefixed on the last derivation step, are imperfective). This part is harder to evaluate as many of the verbs cannot be found on the internet. A possible evaluation method would be to test the unanchored models with various lexical anchors against corpus data, but this requires both the parser that supports frame semantics\index{frame semantics} (to work efficiently with different verbs) and a large corpus that contains complex verbs \index{complex verb}. I leave these tasks for future \index{tense!future} research.
 
\begin{longtable}{lll}
\caption{Output of the XMG\index{XMG} processing for the class of verbs that are prefixed, then possibly suffixed, and then prefixed again \label{table:prefsuffverbs}}\\
...
*perepopopisyvat' & derivational base\index{derivational base} does not exist & perfective \\\midrule
...
*popopopisyvat' & derivational base\index{derivational base} does not exist & perfective\\  \midrule
...
The first of the two verbs, \textit{*perepopisyvat'}, that could have been translated as `to write all of for some time' would be blocked because the interpretation of the prefix \textit{pere-} \index{suffix!pere-@\textsl{pere-}}  related with the cardinality scale\index{scale!cardinality scale}\index{scale} is `performing the action completely with each item in the set'. Now, if the interpretation of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  does not get strengthened (some time $\rightarrow$ all context \index{context} -specified time), we obtain a contradiction.  If it does get strengthened (every writing event is maximal with respect to the corresponding member of the set), then the same semantics can be expressed by the simpler verb \textit{perepisat'} `to write all of'. 

The second verb, \textit{*popopisyvat'}, has a similar semantic structure and also could be translated as `to write all of for some time' (see the discussion about the differences between the distributive \index{prefix interpretation!distributive} interpretations of the prefixes \textit{po-} \index{suffix!po-@\textsl{po-}}  and \textit{pere-} \index{suffix!pere-@\textsl{pere-}}  in Chapter~\ref{Chapter5}). This verb refers to the same set of events as the verb \textit{popisat'} with distributive \index{prefix interpretation!distributive} interpretation (`to write all of'), although the surface semantic representations of the two verbs are different, so a deeper analysis is needed in this case.
...
\section{Result evaluation and comparison \index{prefix interpretation!comparison}}
In order to compare the predictions of my model to that of earlier theories, I have implemented the system proposed in \citet{Tatevosov:09} for exactly the same fragment (one verbal stem, one `lexical' prefix, five prefix-interpretation pairs, the imperfective suffix \index{suffix!imperfective suffix} ). For this part I have omitted the lexical entries for direct objects as they do not influence the interpretation of the prefixed verbs. As the approach is syntactic, all restrictions are formulated in syntactic terms and the frame dimension is used to represent the order of attachment of affixes with different semantics. In this implementation, for example, the class for the distributive \index{prefix interpretation!distributive} interpretation of the prefix \textit{pere-} \index{suffix!pere-@\textsl{pere-}}  looks as shown on \figref{xmg:Tat:pere}. The restriction on this prefix attachment is the imperfective aspect of the base verb, which is reflected via a syntactic constraint on the feature \textit{aspect} here.
...
declare ?VP ?VPInt ?Pere ?PereLex ?AGR ?X0 ?X1 
{
  ...
  <frame>{
    ?X1[distributive \index{prefix interpretation!distributive},
       of: ?X0]
  }
}
\end{lstlisting}
\caption{XMG\index{XMG} implementation for the distributive \index{prefix interpretation!distributive} interpretation of the prefix \textit{pere-} \index{suffix!pere-@\textsl{pere-}}  according to the theory of \citet{Tatevosov:09}\label{xmg:Tat:pere}}
\end{figure}

However, a direct comparison \index{prefix interpretation!comparison} of the predictions of the two models is not possible, as \citet{Tatevosov:09} does not offer any theory about the nature of various interpretations of the imperfective suffix \index{suffix!imperfective suffix} . Two solutions are available in this situation: either introduce both interpretations of the imperfective suffix \index{suffix!imperfective suffix}  in the implementation of the theory proposed by \citet{Tatevosov:09} or count those models produced with the implementation of my theory that differ only with respect to the interpretation only once. The second option requires more manual checking, but is more fair with respect to the analysis of \citet{Tatevosov:09}, so I decided to adopt it.

My implementation of the analysis proposed in \citet{Tatevosov:09} produces 81 models for the same fragment. I have done a full analysis of the resulting models and I would like to show the results from verbs with two prefixes and the verbs that are prefixed after the imperfective suffix \index{suffix!imperfective suffix}  is attached. At the end I will provide the summary with precision, recall, and F-score data for the two models.
...
%3 false predicted by the current approach (to be discarded by the pragmatics\index{pragmatics} module)

These three verbs are verbs that contain the distributive \index{prefix interpretation!distributive} prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  stacked over some other prefix (with non-distributive \index{prefix interpretation!distributive} interpretation): \textit{podopisat'} `to finish writing all of', \textit{poperepisat'} `to rewrite all of', and  \textit{pozapisat'} `to rewrite all of'. They are, according to the theory proposed in \citet{Tatevosov:09}, possible, but not extensively discussed in the paper (a manuscript by the same author, dedicated to this usage of the prefix and cited among the references, never appeared). I personally do not find them acceptable and \citet[143]{Tatevosov:09} himself marks such verbs as ``interpretable with difficulty''. They could be accommodated in my account if the distributive \index{prefix interpretation!distributive} interpretation of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}}  is represented separately and is two-layered, effectively combining in itself the semantics of the imperfective suffix \index{suffix!imperfective suffix}  (iterative/habitual interpretation) and the current representation of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}} . The piece of code shown on \figref{xmg:podistr} implements this solution and allows to produce exactly those three verbs (if the class is combined with verbs that are already prefixed once). One would probably want to associate this representation with a higher cost in comparison \index{prefix interpretation!comparison} with the initial representation I offer if a subsequent pragmatic module is used.
...
declare ?VP ?VPInt ?Po ?PoLex ?AGR ?X0 ?X1 ?Init ?Fin ?VDim
{
  ...
  <frame>{
    ?X1[bounded-event\index{bounded event} & iteration,
...
\end{lstlisting}
\caption{XMG\index{XMG} code for implementing the `coerced' distributive \index{prefix interpretation!distributive} interpretation of the prefix \textit{po-} \index{suffix!po-@\textsl{po-}} \label{xmg:podistr}}
\end{figure}

If the three verbs that we have just discussed are considered existent, then the prefixation \index{morphological operation!prefixation} system proposed by \citet{Tatevosov:09} produces five models that must be discarded. In contrast with my proposal, there is no further explanation of why exactly those verbs (two of them are produced by the same rule that forms the three verbs we have just discussed) would be problematic.

Among the verbs with one or two prefixes and an imperfective suffix \index{suffix!imperfective suffix}  added at the last step of the derivation the number of errors stays close (two versus three), although again constructed but not existent verbs are distinct in two approaches. Both implementations have full recall with respect to this part and the part we have discussed before.

The comparison \index{prefix interpretation!comparison} becomes more interesting when  we consider the most complex verbs \index{complex verb} created by the two implementations. The number of models produced here is close: 45 models according to the analysis by \citet{Tatevosov:09} and 49 models in the implementation of my analysis. The overlap of these sets constitutes, however, only 27 models. The first thing to note is that the group of verbs that are marked as imperfective in Table~\ref{table:prefsuffverbs} cannot be (and is not) produced in the system proposed by \citet{Tatevosov:09}. One may ask whether they should be produced at all: an attentive reader probably noticed that both Table~\ref{table:suffverbs} and Table~\ref{table:prefsuffverbs} contain, for example, the imperfective verb \index{imperfective aspect!imperfective verb} \textit{perezapisyvat'} `to be writing down again'. The structure of the two verbs, however, is different: in one case the imperfective suffix \index{suffix!imperfective suffix}  is attached as the last step of the derivation and in the other case it happens before the repetitive \index{prefix interpretation!repetitive} \textit{pere-} \index{suffix!pere-@\textsl{pere-}}  is attached. On the semantic side this is reflected in what ends up to constitute the preparatory stage of the event: once it is the whole completed event of the same type, and in the second case it is another ongoing/partial event. Another difference is that only in the first structure the habitual interpretation of the suffix is possible. 

The second group of complex verbs \index{complex verb} that is not produced by the implementation of the analysis offered in \citealt{Tatevosov:09} is formed by the verbs with the outermost prefix \textit{do-} \index{suffix!do-@\textsl{do-}} . They follow the pattern we have extensively discussed in Chapter~\ref{Chapter2}.

Among the rest of the models produced by the second implementation are such verbs as \textit{pereperepopisyvat'} with a semantic structure of a distributive \index{prefix interpretation!distributive} interpretation over imperfective of the repetition \index{prefix interpretation!repetitive} of a delimited event. Such semantic structures are hardly conceivable and the corresponding verbs do not exist.\footnote{This judgement is mostly based on introspection and personal communication with other native speakers, as any such verb would be rare and the absence of data on the Internet is not a reliable indicator of the non-existence. I plan to conduct additional experiments in future \index{tense!future} to get statistically reliable evidence about the existence of such complex verbs \index{complex verb}.}
...
%*perepopopisyvat' & derivational base\index{derivational base} does not exist & perfective \\ \hline  \hline
%
%
...
%*popopopisyvat' & derivational base\index{derivational base} does not exist & perfective\\ \hline
%
...
%\end{tabular}
%\caption{Output of the XMG\index{XMG} processing for the class of verbs that are prefixed, then possibly suffixed, and then prefixed again {table:prefsuffverbs}}
%\end{table}
...
\begin{itemize}
\item count 79 models instead of 88 for my analysis by removing such models that differ only with respect to the interpretation of the secondary imperfective \index{imperfective aspect!secondary imperfective};
\item count the models for the existence of which I argue in Chapter~\ref{Chapter2} as correct (imperfective verbs \index{imperfective aspect!imperfective verb \index{imperfective aspect!imperfective verb}} formed when the last affix attached is the repetitive \index{prefix interpretation!repetitive} \textit{pere-} \index{suffix!pere-@\textsl{pere-}}  and perfective \textit{do-} \index{suffix!do-@\textsl{do-}} prefixed verbs)

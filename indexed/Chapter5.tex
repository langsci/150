...
A simple illustration is provided by \ref{ex:do:varit}. The verb \textit{varit'}$^{\IPF}$ `to cook' lexicalises a scale with the \isi{maximum point} corresponding to \textit{fully cooked} and the prefix \Prefix{do-} contributes information that at the end of the event this point is reached. 
...
After that, in Chapter~\ref{Chapter7}, I will offer a formalization of the intuitions and observations laid out in this chapter, using the combination of Frame Semantics (\citealt{Fillmore:82}) and \isi{LTAG} (\citealt{JoshiSchabes:97}) formalised in \citealt{KallmeyerOsswald:13}. 
